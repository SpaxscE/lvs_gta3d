...
function ENT:UpdatePoseParameters( steer, speed_kmh, engine_rpm, throttle, brake, handbrake, clutch, gear, temperature, fuel, oil, ammeter )
	...
	local Jiggle = math.Clamp( self._JiggleVel / 10,-1.1,1.1) + math.Clamp( math.cos( CurTime() * 10 + self:EntIndex() * 1.337 ) * self._smOldJiggle * 10, -0.3, 0.3 )

	self._smJiggle = self._smJiggle + (Jiggle - self._smJiggle) * FT * 8
