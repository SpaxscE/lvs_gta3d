
ENT.Base = "lvs_wheeldrive_gta3d"

ENT.PrintName = "Sweeper"
ENT.Author = "Digger"
ENT.Information = ""
ENT.Category = "[LVS] GTA3D - Public Service"

ENT.VehicleCategory = "GTA3D"
ENT.VehicleSubCategory = "Public Service"

ENT.Spawnable			= true
ENT.AdminSpawnable		= false

ENT.SpawnNormalOffset = 40

ENT.MDL = "models/diggercars/gtasa/sweeper/sweeper.mdl"

ENT.GibModels = {
	"models/diggercars/gtasa/sweeper/bump_front_dam.mdl",
	"models/diggercars/gtasa/sweeper/bump_rear_dam.mdl",
	"models/diggercars/gtasa/sweeper/door_lf_dam.mdl",
	"models/diggercars/gtasa/sweeper/door_rf_dam.mdl",
}

ENT.AITEAM = 0

ENT.MaxVelocity = 500

ENT.EngineTorque = 100
ENT.EngineIdleRPM = 1000
ENT.EngineMaxRPM = 6000

ENT.TransGears = 0

ENT.HornSound = "gta3d/horns/horn_006_112.wav"
ENT.HornPos = Vector(30,0,30)

ENT.SirenPos = Vector(30,0,30)
ENT.SirenSound = {
	[1] = {
		siren = "common/null.wav",
	},
}

ENT.EngineSoundsSA = {
	idle = {
		sound = "gta3d/engines/sweeper/idle.wav",
		Pitch = 100,
		PitchMul = 60,
		SoundLevel = 65,
	},
	gears = {
		sound = "gta3d/engines/sweeper/gear.wav",
		soundDuration = 1,
		speed = { 1 },
		SoundLevel = 75,
		UseDoppler = true,
	},
	cruise = {
		sound = "gta3d/engines/sweeper/cruise.wav",
		Pitch = 20,
		PitchMul = 90,
		SoundLevel = 75,
		UseDoppler = true,
	},
	throttle_off = {
		sound = "gta3d/engines/sweeper/throttleoff.wav",
		SoundLevel = 75,
	},
	reverse_on = {
		sound = "gta3d/share/reverse_on.wav",
		Pitch = 90,
		PitchMul = 20,
		SoundLevel = 65,
	},
	reverse_off = {
		sound = "gta3d/share/reverse_off.wav",
		Pitch = 80,
		PitchMul = 20,
		SoundLevel = 65,
	},
}

ENT.Lights = {

	{
		Siren = true,
		Trigger = "10",
		Sprites = {
			{ pos = "siren1", colorG = 0, colorB = 0, colorA = 255, width = 100, height = 60 },
		},
	},
	{
		Siren = true,
		Trigger = "01",
		Sprites = {
			{ pos = "siren2", colorG = 0, colorB = 0, colorA = 255, width = 100, height = 60 },

		},
	},

	{
		Trigger = "main+high",
		SubMaterialID = 12,
		Sprites = {
			{  pos = "a_fl", width = 125, colorB = 200, colorA = 150, bodygroup = { name = "l_fl", active = { 0 } }, },
			{  pos = "a_fr", width = 125, colorB = 200, colorA = 150, bodygroup = { name = "l_fr", active = { 0 } }, },

		},
	},

	{
		Trigger = "main",
		ProjectedTextures = {
			{  pos = "a_fl", colorB = 200, colorA = 150, shadows = true, bodygroup = { name = "l_fl", active = { 0 } }, },
			{  pos = "a_fr", colorB = 200, colorA = 150, shadows = true, bodygroup = { name = "l_fr", active = { 0 } }, },
		},
	},

	{
		Trigger = "main+brake",
		SubMaterialID = 16,
		Sprites = {
			{  pos = "a_rl", width = 40, colorG = 0, colorB = 0, colorA = 150, bodygroup = { name = "l_rl", active = { 0 } }, },
			{  pos = "a_rr", width = 40, colorG = 0, colorB = 0, colorA = 150, bodygroup = { name = "l_rr", active = { 0 } }, },
		},
	},

	{
		Trigger = "turnright",
		SubMaterialID = 15,
		Sprites = {
			{  pos = "a_frt", colorG = 100, colorB = 0, colorA = 150, bodygroup = { name = "l_frh", active = { 0 } }, },
			{  pos = "a_rrt", colorG = 0, colorB = 0, colorA = 150, bodygroup = { name = "l_rr", active = { 0 } }, },
		},
	},
	{
		Trigger = "turnleft",
		SubMaterialID = 17,
		Sprites = {
			{  pos = "a_flt", colorG = 100, colorB = 0, colorA = 150, bodygroup = { name = "l_flh", active = { 0 } }, },
			{  pos = "a_rlt", colorG = 0, colorB = 0, colorA = 150, bodygroup = { name = "l_rl", active = { 0 } }, },
		},
	},	
	
	{
		Trigger = "high",
		ProjectedTextures = {
			{  pos = "a_fl", colorB = 200, colorA = 150, shadows = true, bodygroup = { name = "l_fl", active = { 0 } }, },
			{  pos = "a_fr", colorB = 200, colorA = 150, shadows = true, bodygroup = { name = "l_fr", active = { 0 } }, },
		},
	},
	{
		Trigger = "reverse",
		SubMaterialID = 14,
		Sprites = {

			{ pos = "a_rlr", width = 25, height = 25, colorA = 150, bodygroup = { name = "l_rl", active = { 0 } }, },
			{ pos = "a_rrr", width = 25, height = 25, colorA = 150, bodygroup = { name = "l_rr", active = { 0 } }, },

		}
	},	

}

